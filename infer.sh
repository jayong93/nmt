...
nvidia-docker run --rm -v "${SOURCE_DIR}:${VOLUME_DIR}" -v "${INPUT_DIR}:${INNER_INPUT_DIR}" -v "${OUTPUT_DIR}:${INNER_OUTPUT_DIR}" -v "${MODEL_DIR}:${INNER_MODEL_DIR}" -v "${MODEL_DIR}:${INNER_VOCAB_DIR}" tensorflow/tensorflow:nightly-gpu-py3 bash -c "export PYTHONIOENCODING=UTF-8 && cd ${VOLUME_DIR} && python3 -m nmt.nmt \
